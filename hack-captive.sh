...
#interface="$(ip -4 route show to default | awk '/dev/ {print $5}')"
#interface="$( sudo ip -4 route get default  | cut -d "d" -f -2 | cut -d "a" -f 3- | cut  -d " " -f 2-)"
interface="$(sudo ip  -4 route get  default | awk '/dev/ {print $5}')"
#localip="$(ip   -4 route get 1 )"
localip="$(ipconfig getifaddr en0)"
#wifissid="$(iw dev "$interface" link | awk '/SSID/ {print $NF}')"
#airport en0 -I |  awk '/SSID/ {print $2" "$3 " " $4 " " $5}'
#wifissid="$( airport en0 -I |  awk '/SSID/ {print $NF " " $3 " " $4 " " $5}')"
airport en0 -I |  awk '/SSID/ {print $NF " " $3 " " $4 " " $5}'
printf " please enter the wifi network name (SSID) and the wifi network mac address which can "
printf "be found above "^"  **please keep in mind that you might need to enter in a leading 0 if "
read -p "there is not 2 digets in the front its hex des**" wifi
echo $wifi
wifissid=$(echo $wifi)

#gateway="$(ip  -4 route show to default | awk '/via/ {print $3}')"
#gateway="$(ip  -4 route get default | cut -d "d" -f -2 | cut -d "a" -f 3- | cut  -d " " -f 2-)"
gateway="$(sudo ip  -4 route get default | awk '/via/ {print $3}')"
#broadcast="$(ip  -4 addr show dev "$interface" | awk '/brd/ {print $6}')"
broadcast="$(ip  -4 addr show dev | /usr/bin/awk '/brd/ {print $4}')"
#ipmask="$(ip -4 addr show dev "$interface" | awk '/inet/ {print $4}')"
#ipmask="$(ip  -4 addr show dev | /usr/bin/awk '/inet/ {print $2}' | sed -n '2p' )"
ipmask="$( ip  -4 addr show dev | /usr/bin/awk '/inet/ {print $2}' | grep -v 127.0.0.1/8)"
#netmask="$(printf "%s\n" "$ipmask" | cut -d "/" -f 2)"
...
network="$netaddress/$netmask"
#macaddress="$(ip  addr show dev "$interface" \
